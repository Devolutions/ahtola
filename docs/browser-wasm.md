...
| --- | --- |
| `SELECT column-list \| * FROM one-ordinary-rowid-base-table [WHERE integer-primary-key = integer-literal] [LIMIT n]` | Supported: ascending rowid scan or a direct rowid b-tree seek for the optional equality predicate. Both are page-bounded, validate visited pages lazily, and apply `LIMIT` to matching rows. |
| Other `WHERE` forms, joins, subqueries, `ORDER BY`, `GROUP BY`/`HAVING`, `DISTINCT`, window definitions, `OFFSET`, expressions beyond a plain column reference, generated columns | `AhtolaBrowserBoundedQueryException`, naming the exact unsupported construct — natural follow-on slices |
