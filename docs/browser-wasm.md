...
| `SELECT column-list \| * FROM one-WITHOUT-ROWID-base-table [ORDER BY primary-key-prefix [ASC \| DESC]] [LIMIT n [OFFSET m]]` | Supported for ascending BINARY primary keys only. Traverses the table's index b-tree forward or backward in primary-key order, including records stored in interior cells and overflowing records; remaps the physical PK-first record to logical column order. An explicit uniformly ascending or descending prefix needs no extra sort (primary-key columns are non-NULL). A secondary index does not change the base scan. Other declared PK collation/direction, `WHERE`, non-key or mixed-direction `ORDER BY`, and `INDEXED BY` shapes reject before scanning. No rowid is exposed. |
