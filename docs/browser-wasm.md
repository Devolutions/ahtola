...
| --- | --- |
| `SELECT column-list \| * FROM one-ordinary-rowid-base-table [WHERE integer-primary-key <comparison> integer-literal [AND ...] \| integer-primary-key BETWEEN integer-literal AND integer-literal] [ORDER BY integer-primary-key [ASC \| DESC]] [LIMIT n [OFFSET m]]` | Supported comparisons: `=`, `<`, `<=`, `>`, `>=` (including literal-first forms); `BETWEEN` is inclusive. Equality performs a point seek; ranges seek their starting bound and stop at the opposite bound in the requested direction. Non-negative literal `OFFSET` skips matching rows before `LIMIT` counts emitted rows; reads remain page-bounded and validate visited pages lazily. |
