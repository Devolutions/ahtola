...
| Partial prefix bootstrap (`PartialBootstrap.Prefix`) | **Qualified (eager-only)** | The initial request carries Turso's tag-5 portable RoaringBitmap selector for complete 4 KiB pages in the requested prefix. The staged image is published only when the selected range covers the server-declared database size; otherwise open fails before local state is installed because no lazy page-fault storage exists. |
| Query bootstrap and lazy loading (`QueryPages`, `SegmentSize`, `Prefetch`) | **Explicitly rejected** | Query-selected page sets and lazy page faults are not implemented. Validation rejects these modes before network or local-state mutation. |
...
| `sync-no-embedded-sync-engine-port` | partial | s2-capability | L | 0 | 0 | The managed provider now implements raw bootstrap/page pulls, eager prefix selection for complete images, pull-only MVCC logical replay, durable local push journaling, protocol metadata, and atomic publication. Query/lazy sync, zstd, remote encryption, and Turso's complete conflict/checkpoint policy remain outside the managed subset. |
...
| `sync-no-page-protocol-pull-decode` | partial | s2-capability | M | 0 | 0 | Raw 4-KiB page bootstrap/incremental streams and protocol-2 full ReplaceBase fallback are decoded and atomically published. Initial eager prefix selection is encoded with Turso's page-selector protocol; zstd remains explicitly rejected. |
| `sync-no-partial-sync-lazy-page-storage` | missing | s2-capability | L | 0 | 0 | Eager prefix selection is supported only when it materializes the complete database. Ahtola still has no lazy page-fault storage, so an incomplete prefix fails before publication, while server-side query selection, segment loading, and prefetch remain explicitly rejected. |
