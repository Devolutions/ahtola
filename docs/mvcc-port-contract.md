...
| **2** | Durable logical log (`*.db-log`) with Turso LML2/MVTX framing constants, CRC32C, upsert/delete ops, replay into `MvStore` on enable; encrypted databases AES-GCM-encrypt row payload chunks while authenticating salt/length/op-count/commit-ts/chunk metadata; checkpoint TRUNCATE clears log |
