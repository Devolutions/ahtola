...
try {
    ...
    try {
        ...
        if ($BrowserEngine -eq 'chromium') {
            $expectedStatus = 'PASS:capabilities=True;storage=True;crypto=True;ado=42;persistent-ado=126;persistent-core=210;persistent-features=546;persistent-fts=8;browser-memory=504;reserved-path-rejected=True;worker-isolation=3;persistent-blob=01020908;persistent-backup=336;persistent-inbound-backup=378;persistent-encrypted=420;encrypted-bounded=36;encrypted-maintenance=9368;persistent-encrypted-raw-key=462;encrypted-wrong-key-rejected=True;encrypted-cipher-mismatch-rejected=True;sync-read-mirror=True;vector-search=195-1987-643'
            & node $probeScript $uri $expectedStatus
        } else {
